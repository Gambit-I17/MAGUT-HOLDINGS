...
/* ----------------------------------------------------------------
   ...
   - Auto-scrolls every 4.5s
   - Supports arrows and dots
   - Pauses on hover
   ---------------------------------------------------------------- */
function initHomeGalleryCarousel() {
  var carousel = document.getElementById('homeGalleryCarousel');
  var dotsWrap = document.getElementById('homeGalleryDots');
  if (!carousel || !dotsWrap) return;

  var track = carousel.querySelector('.home-gallery-track');
  var slides = carousel.querySelectorAll('.home-shot');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var revealEls = document.querySelectorAll(
    '.card, .pcard, .news-card, .team-card, .val-card, .part-card, .v-box, .metric-badge, .sector-row'
  );
